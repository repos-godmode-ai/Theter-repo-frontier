...
Public Solana helper endpoints (no admin token): `GET /v1/solana/mint-info`, `POST /v1/solana/derive-ata`. Product creation validates `merchantPayTo` on-chain against `USDT_MINT` (see [Solana token docs](https://solana.com/docs/tokens)). For local smoke tests when your RPC cluster does not have that account, set **`PAYWALL_SKIP_PAY_TO_VALIDATION=1`** (never in production).
